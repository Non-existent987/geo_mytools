...
def linestrings_intersect(line1, line2):
    ...
    geojson中的行字符串是否相互交叉有效。
    参考:http://www.kevlindev.com/gui/math/intersection/Intersection.js
    关键字参数:
    line1——第一行geojson对象
    line2——第二行geojson对象
    如果(line1与other相交)返回相交点数组else空数组
    """
    intersects = []
...
def point_in_polygon(point, poly):
    ...
    点—点geojson对象
    多边形geojson对象
    if(point inside poly)返回true else false
    """
...
def point_in_multipolygon(point, multipoly):
    ...
    点是否位于多多边形中有效(不支持甜甜圈多边形)
    关键字参数:
    点—点geojson对象
    多多边形geojson对象
    if(point inside multipoly)返回true else false
    """
...
def geometry_within_radius(geometry, center, radius):
    ...
    无论点、线串或多边形在以中心为中心的半径内，都是有效的
    关键字参数:
    几何——点/linstring/多边形geojson对象
    中心点geojson对象
    半径,半径
    如果(几何内部半径)返回真，否则为假
    """
...
def centroid(poly):
    ...
    求多边形的质心
    改编自http://paulbourke.net/geometry/polyarea/javascript.txt
    关键字参数:
    多边形geojson对象
    返回多边形重心
    """
...
def destination_point(point, brng, dist):
    ...
    pt——多边形geojson对象
    brng——角度
    dist——目的地到基点之间的距离，单位为米
    返回终点对象
    """
    dist = float(dist) / 6371 /1000 # convert dist to angular distance in radians
...
def destination_point2(lon,lat, azimuth, dist):
    """
    Calculate a destination Point base on a base point and a distance
    Keyword arguments:
    pt   -- polygon geojson object
    azimuth -- an angle in degrees
    dist -- distance in Kilometer between destination and base point
    return destination point object
    根据一个基点和一个距离计算一个终点基点
    关键字参数:
    ...
    lon2 = lon1 + math.atan2(math.sin(azimuth) * math.sin(dist) *
                             math.cos(lat1), math.cos(dist) - math.sin(lat1) * math.sin(lat2))
    lon2 = (lon2 + 3 * math.pi) % (2 * math.pi) - math.pi  # normalise to -180 degree +180 degree
...
def simplify(source, kink=20):
    """
     ...
     geojson点的源[]数组
    扭结以米为单位，在这个深度以上的扭结保持不变
    扭结深度是三角形abc的高度，其中a-b和b-c是两个连续的线段
    """
