...
class RoundProgress(QMainWindow,Ui_MainWindow):
    ...
    def __init__(self):
        ...
        #--样式设置--
        #yunxing 样式
        self.pushButton.setStyleSheet(MyStyleSheet.button_style_ico())
        play_icon = qta.icon("fa.play-circle-o",)
        self.pushButton.setIcon(play_icon)
        self.pushButton.setIconSize(QtCore.QSize(100, 100))
        self.pushButton.setText(QtCore.QCoreApplication.translate("MainWindow", ""))
        ...
        #主页样式
        self.centralwidget.setStyleSheet(MyStyleSheet.button_style_QComboBox())
    ...
    def gengxin(self,i):
        if i ==0: 
            ...
            QMessageBox.information(self, "提示", "发生错误，报错信息为：{}".format(MyThread.except_e))
            self.stackedWidget.setCurrentIndex(0)
    #多线程信号传递使用
    def jindutiao(self):
        print(tm(),'程序开始：')
    ...
    def yunxing_sheet_0(self):
        ...
        if (MyThread.data_xianwang.shape[0]>1)& (MyThread.data_wenti.shape[0]>1):
            ...
            MyThread.juli = self.spinBox.value() #获取计数器的值
...
# ------------------------------------------------------------                
            ...
            try:
                ...
                print(tm(),'错误为',e)
                QMessageBox.warning(self, "标题", "请检查经纬度数据,错误提示{}".format(e)) 
...
class MyThread(QThread):
    ...
    #问题点变量
    wenti_lon=wenti_lat=xianwang_lon=xianwang_lat=juli=''    # p = 0
    except_e = None
    def __init__(self):
    ...
    def printABCD(self):
        try:
            ...
            if self.sending_button == 'pushButton':
                ...
                print(tm(),'文件copy')
                data = self.data_wenti.copy()
                data_5_lie = list(data.columns)
                data = data.rename(columns={self.wenti_lon:'5G经度',self.wenti_lat:'5G纬度'})
                print(tm(),'5G小区扩展距离-开始...')
                data_buff = add_buffer(data,
                    lon='5G经度',
                    lat='5G纬度',
                    buff_float= int(self.juli))
                print(tm(),'5G小区扩展距离-完成')
                print(tm(),'copy-4G数据')
                data_4 = self.data_xianwang.copy()
                data_4_lie = list(data_4.columns)
                data_4 = data_4.rename(columns={self.xianwang_lon:'经度',self.xianwang_lat:'纬度'})
                print(tm(),'4G数据制作图层')
                data_p = add_points(data_4,
                    lon='经度',
                    lat='纬度'
                    )
                print(tm(),'4G数据制作图层完成')
                print(tm(),'开始数据sjoin')
                res = gpd.sjoin(data_buff,data_p)
                print(tm(),'sjoin结束')
                print(tm(),'删除无用的列')
                res = res.drop(columns=['geometry','index_right'])
                print(res.columns)
                res = distancea_df(res,
                        '5G经度', 
                        '5G纬度', 
                        '经度', 
                        '纬度',
                ...
                res.to_excel(os.path.split(self.data_wenti_file)[0]+"/5G邻区匹配结果.xlsx",encoding='gbk',index=False)
                time.sleep(2)
                #-------------输出--------------
                print(tm(),"=======================计算完成======================")
            ...
            print(tm(),"程序报错:{}".format(e))
            self.except_e = traceback.print_exc()
            print('错误信息为',self.except_e)
...
if __name__ == '__main__':
    ...
    icon = QtGui.QIcon()
    icon.addPixmap(QtGui.QPixmap(":/poto/haidao.ico"),QtGui.QIcon.Normal, QtGui.QIcon.Off)
